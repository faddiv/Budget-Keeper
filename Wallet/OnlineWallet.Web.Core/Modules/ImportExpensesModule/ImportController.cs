...
namespace OnlineWallet.Web.Modules.ImportExpensesModule
{
    ...
    public class ImportController : Controller
    {
        ...
        public List<ExportImportRow> ProcessTransactions(IFormFile file)
        {
            ...
            var query = _db.MoneyOperations.AsNoTracking().Where(e => from <= e.CreatedAt && e.CreatedAt <= to);
            var wallets = _db.Wallets.AsNoTracking().ToList();
            if (directions.Count > 0)
            {
                var predicate = PredicateBuilder.New<MoneyOperation>();
            ...
            var savedItems = query.OrderBy(e => e.MoneyOperationId).ToList();
            foreach (var item in list)
            {
                ...
                if (index > -1)
                {
                    ...
                    item.MatchingId = savedItem.MoneyOperationId;
                }
            }
            return list;
        }
