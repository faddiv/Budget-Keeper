...
export function validate<TState, TProps>(
  config: ValidationConfig<TState, TProps>,
  ...
  for (const key in config) {
    ...
    const nextValue = state ? (elementConfig.valueGetter ? elementConfig.valueGetter(state, props) : defaultGetter(state, key)) : "";
    ...
    for (const validatorC of elementConfig.validators) {
      ...
      if (validatorF.paramCount && validatorF.paramCount > 1) {
        ...
        isValid = validatorF.call(undefined, values);
      } else {
