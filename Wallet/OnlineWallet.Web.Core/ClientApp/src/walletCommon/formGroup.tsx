    ...
    return (
        <div className="form-group row">
            ...
            <div className="col-sm-10">
                {rest.children ? rest.children : defaultInput(type || "", id, name, label, value, onChange, autoComplete || false, validation)}
            </div>
        </div>
    );
};

function defaultInput(type: string, id: string, name: string, label: string, value: any, onChange: (() => void) | undefined, autoComplete: boolean, validation: ValidationStateElement | undefined): Array<React.ReactElement<any>> {
    const elements: Array<React.ReactElement<any>> = [
        <input key={1} type={type} className={classNames("form-control", { "is-invalid": validation && validation.showError })}
            id={id} name={name} placeholder={label} value={value} onChange={onChange} autoComplete={autoComplete ? undefined : "off"} />
    ];
    if (validation) {
        elements.push(
