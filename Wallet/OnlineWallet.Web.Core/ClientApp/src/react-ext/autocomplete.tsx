...
export class Autocomplete extends Component<AutocompleteProps, AutocompleteState> {
  ...
  onlyItemActive() {
    return this.state.items.length === 1 && this.state.items[0] === this.state.active;
  }

  navigate(direction: number) {
    let ok = false;
    if (this.state.open) {
      const index = this.state.active ? this.state.items.indexOf(this.state.active) + direction : -1;
  ...
  hasSelectable() {
    return this.state.items.length > 0 && (this.state.items.length > 1 || this.getItemValue(this.state.items[0]) !== this.state.value);
  }
  ...
  renderItem(item: AutocompleteModel, active: boolean) {
    return (
      <li
        ...
        dangerouslySetInnerHTML={{ __html: item.nameHighlighted || "" }}
      ></li>
    );
  }
