...
export function MonthSelector({ year, month }: MonthSelectorProps) {
  ...
  return (
    <div className="btn-group">
      <Link className="btn btn-light" to={`/transactions/${year - 1}/${12}`}>
        {year - 1}
      </Link>
      ...
      {renderRange(1, 12, (i) => (
        <Link key={i} className={getMonthColoring(year, month, currentYear, currentMonth, i)} to={`/transactions/${year}/${i}`}>
          {i}
        </Link>
      ))}
      <Link className="btn btn-light" to={`/transactions/${year + 1}/${1}`}>
        {year + 1}
      </Link>
    </div>
  );
