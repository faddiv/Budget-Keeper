...
export class WalletService {
    ...
    update(wallet: Wallet): Observable<Wallet> {
        return decorateCommonCatch(this.api.put(<any>wallet.moneyWalletId, wallet));
    }

    insert(wallet: Wallet): Observable<Wallet> {
        return decorateCommonCatch(this.api.post(undefined, wallet.name));
    }

    delete(wallet: Wallet): Observable<any> {
        return decorateCommonCatch(this.api._delete(<any>wallet.moneyWalletId));
    }
}