...
namespace OnlineWallet.Web.Modules.GeneralDataModule
{
    ...
    public class ArticleControllerGetByTests : ServiceTestBase
    {
        ...
        {
            await Fixture.PrepareDataWith(tr => tr.WithCategoryRandom()
        ...
        public async Task Returns_with_the_most_recent_wallet()
        {
            await Fixture.PrepareDataWith(tr => tr
                .TheFirst(3).WithName("alfa1")
                .TheNext(3).WithName("alfa2")
                .TheFirst(1).WithCreatedAt("2017-10-15").WithWallet(Fixture.WalletCash)
                .TheNext(1).WithCreatedAt("2017-10-16").WithWallet(Fixture.WalletBankAccount)
                .TheNext(1).WithCreatedAt("2017-10-14").WithWallet(Fixture.WalletCash)
                .TheNext(1).WithCreatedAt("2017-10-14").WithWallet(Fixture.WalletCash)
                .TheNext(1).WithCreatedAt("2017-10-15").WithWallet(Fixture.WalletBankAccount)
                .TheNext(1).WithCreatedAt("2017-10-13").WithWallet(Fixture.WalletCash)
            );
            ...
            result.Should()
                .HaveCount(2).And
                .OnlyContain(e => e.LastWallet == Fixture.WalletBankAccount.MoneyWalletId);
        }
